...
const doSignup = createServerFn({ method: "POST" })
  .validator((d: { email: string; password: string }) => d)
  .handler(async ({ data }) => {
    const { signupUser } = await import("~/lib/auth-server");
    return signupUser(data.email, data.password);
  });
...
function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await doSignup({ data: { email, password } });
      if (result.ok) {
    ...
    <div className="min-h-dvh bg-white dark:bg-gray-950 flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-950">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Start your 7-day free trial today.</p>
